...
found:
  p->state = EMBRYO;
  p->pid = nextpid++;
  p->priority = 0; // Initialize the process with a priority of 0.
...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    // Re-initialize priority trackers.
    maxPriority = -1;
    maxPriorityProc = ptable.proc;
    ...
    c->proc = maxPriorityProc;
    switchuvm(maxPriorityProc);
    maxPriorityProc->state = RUNNING;

    swtch(&(c->scheduler), maxPriorityProc->context);
    switchkvm();
